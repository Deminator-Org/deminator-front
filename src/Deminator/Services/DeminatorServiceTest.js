...
class DeminatorService {
  ...
  getGameSetup(){
    ...
    return {width: this.width, height: this.height, grid: grid, firstClick: true, player:0};
  }
  ...
  onClick(pos){
    if(this.grid[pos].isOpen){
      if(DeminatorUtil.isCompleted(pos, this.grid, this.width, this.height)){
        DeminatorUtil.forAroundCel(pos, (posPlus)=>!this.grid[posPlus].isOpen && !this.grid[posPlus].display, (posPlus)=>this.openCel(posPlus), this.width, this.height);
      }
    }
    else{
      this.openCel(pos);
    }
    return {width: this.width, height: this.height, grid: this.grid, firstClick: false};
  }
